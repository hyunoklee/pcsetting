...
YoloObjectDetector::YoloObjectDetector(ros::NodeHandle nh)
    : nodeHandle_(nh),
      ...
      rosBoxes_(0),
      rosBoxCounter_(0),
         imagergb_sub(imageTransport_,"/om_with_tb3/camera/rgb/image_raw",1),       //For depth inclussion
         imagedepth_sub(imageTransport_,"/om_with_tb3/camera/depth/image_raw",1),   //For depth inclussion
  ...
  void YoloObjectDetector::cameraCallback(const sensor_msgs::ImageConstPtr& msg, const sensor_msgs::ImageConstPtr& msgdepth)
{
  ...
  ROS_DEBUG("[YoloObjectDetector] USB image received.1");
  try {
  ...
  ROS_DEBUG("[YoloObjectDetector] USB image received.2");
...
void *YoloObjectDetector::detectInThread()
{
 ...
 if (!publishLabelImage(label_im)) {
    ROS_DEBUG("Label image has not been broadcasted.1");
  }
  ...
  ROS_DEBUG("Label image has not been broadcasted.2");

  label_im.release();
  ROS_DEBUG("Label image has not been broadcasted.3");
  free_detections(dets, nboxes);
  ROS_DEBUG("Label image has not been broadcasted.4");
  demoIndex_ = (demoIndex_ + 1) % demoFrame_;
  ROS_DEBUG("Label image has not been broadcasted.5");
  running_ = 0;
  return 0;
}
